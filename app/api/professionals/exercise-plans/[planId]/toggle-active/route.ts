 ...
 * Toggle the active status of an exercise plan
 */
export async function PATCH(
  ...
  try {
    ...
    if (userData.role !== "trainer") {
      return NextResponse.json(
        { error: "Only trainers can modify exercise plans" },
        { status: 403 }
      );
    }
    ...
    const { data: planData, error: planError } = await adminSupabase
      .from("exercise_plans")
      .select("id, trainer_id, client_id, is_active")
      .eq("id", planId)
      .single();
    ...
    if (planData.trainer_id !== user.id) {
      return NextResponse.json(
        { error: "You can only modify your own exercise plans" },
        { status: 403 }
      );
    }
    ...
    console.error(
      "Error in PATCH /api/professionals/exercise-plans/[planId]/toggle-active:",
      error
    );
    return NextResponse.json(
